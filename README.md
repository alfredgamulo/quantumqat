...
- **Dependabot PRs can't sign, so their CI signing step is skipped by design.** GitHub deliberately withholds Actions secrets from Dependabot-triggered `pull_request` runs (so a malicious dependency bump can't exfiltrate them), which means `SIGNING_SECRET` is empty on those runs. To keep that from failing the build, [`build.yml`](.github/workflows/build.yml) only passes the cosign key on non-PR events (`github.event_name != 'pull_request' && secrets.SIGNING_SECRET || ''`) — Dependabot PRs still validate that the image builds, they just don't sign. The real sign+push runs on the `push` to `main` after merge. So a Dependabot PR whose only red check is the cosign/signing step is expected, not broken.
